...
def load_bbo(ticker, date, save = False):
    ...
    # INDEX IS NOW BY MINUTE
    # df_bbo_cleaned.index = df_bbo_cleaned.index.floor('min')
...
## Obsolete : majority of values were Nans
...
def load_all(start_date, end_date, save = False):  
...
def compress_to_tar(input_folder, output_folder):
    """
    ...
    print(f"Compressing folder '{input_folder}' into '{output_folder}'...")
    
    # Create a .tar file
    with tarfile.open(output_folder, "w") as tar:
        tar.add(input_folder, arcname=os.path.basename(input_folder))
    
    print(f"Folder successfully compressed into '{output_folder}'.")
    return None
